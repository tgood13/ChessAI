...
def evaluate(board, maximizing_color):
    ...
    :param board: the current board being used for the game (Board)
    :param maximizing_color: color associated with maximizing player
...
def minimax3(board, depth, alpha, beta, maximizing_player, maximizing_color):
    """
    minimax algorithm used to find best move for the AI
    :param board: the current board being used for the game (Board)
    :param depth: controls how deep to search the tree of possible moves (int)
    ...
    if maximizing_player:
        ...
        for move in moves:
            board.make_move(move[0], move[1])
            current_eval = minimax3(board, depth - 1, alpha, beta, False, maximizing_color)[1]
            board.unmake_move()
            if current_eval > max_eval:
                max_eval = current_eval
                best_move = move
        ...
        for move in moves:
            board.make_move(move[0], move[1])
            current_eval = minimax3(board, depth - 1, alpha, beta, True, maximizing_color)[1]
            board.unmake_move()
            if current_eval < min_eval:
                min_eval = current_eval
                best_move = move
